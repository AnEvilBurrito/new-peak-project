...
def save_results_to_s3(
    ...
    if results_df is not None and len(results_df) > 0:
        ...
        logger.info(f"✅ Saved results to S3: {results_path}")
    ...
    if summary_df is not None and len(summary_df) > 0:
        ...
        logger.info(f"✅ Saved summary stats to S3: {summary_path}")
    ...
    if failed_tasks_df is not None and len(failed_tasks_df) > 0:
        ...
        logger.info(f"✅ Saved failed tasks to S3: {failed_path}")
...
def main():
    ...
    logger.info("🚀 Starting ML Baseline Evaluation Runner (Configuration Version)")
    ...
    try:
        ...
        if csv_path is None:
            # Auto-discovery mode
            logger.info("🚀 Auto-discovery mode: Finding task lists based on MODEL_NAMES")
            ...
            if s3_manager is None:
                s3_manager = S3ConfigManager()
                logger.info("✅ S3 manager initialized for auto-discovery")
            ...
            logger.info(f"📁 Manual mode: Loading CSV from {csv_path}")
        ...
        for model_name, model_data in overall_results.items():
            ...
            logger.info(f"\n  Model: {model_name}")
            logger.info(f"    Status: {'✅ Success' if model_data['results'] is not None else '❌ Failed'}")
            if model_data["results"] is not None:
        ...
        logger.info("🎉 ML Baseline Evaluation completed successfully!")
        ...
        logger.error(f"❌ ML Baseline Evaluation failed: {e}")
        logger.error(traceback.format_exc())
