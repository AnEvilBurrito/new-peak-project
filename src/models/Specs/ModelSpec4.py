...
class ModelSpec4:
    ...
    def generate_forward_archtype_and_regulators(self, specie):
        ...
        rate_law = create_archtype_michaelis_menten_v2(stimulators=0,
    ...
    def generate_forward_archtype_and_regulators_basal(self, specie):
        all_regulations = [(reg.from_specie, reg.to_specie) for reg in self.regulations]
        all_regulation_types = [reg.reg_type for reg in self.regulations]            
        regulators_for_specie = []
        for i, reg in enumerate(all_regulations):
        ...
        rate_law = create_archtype_basal_michaelis(stimulators=0,
                                                    stimulator_weak=total_up_regulations,
                                                    allosteric_inhibitors=total_down_regulations,
                                                    competitive_inhibitors=0)
    ...
    def generate_network(self, network_name, mean_range_species, rangeScale_params, rangeMultiplier_params, 
                         random_seed=None, receptor_basal_activation=True) -> ModelBuilder:
        '''
        ...
        def add_reactions(specie, model: ModelBuilder, basal=False):
            '''
            Returns the forward and reverse reactions for a given specie.
            '''
            forward_reaction = self.get_forward_reaction(specie, mean_range_species, rangeScale_params, 
        ...
        for receptor in self.receptors:
            add_reactions(receptor, model, basal=receptor_basal_activation)
        ...
        model.precompile()
        logger = logging.getLogger(__name__)
        logger.debug(f"Generated model {network_name} with {len(model.reactions)} reactions.")
        logger.debug(f"Model States: {len(model.states)}")
        logger.debug(f"Model Parameters: {len(model.parameters)}")
        logger.debug(f"Model Reactions: {len(model.reactions)}")
        logger.debug('\n--- Antimony Model ---\n')
    ...
    def get_forward_reaction(self, specie, mean_range_species, rangeScale_params, rangeMultiplier_params, rng,
                             basal=False):
        '''
        Returns the forward reaction for a given specie. Basal indicates if the reaction always has a basal rate forward
        controlled by a Kc constant.
        '''
        if basal:
